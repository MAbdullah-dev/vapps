  ...
  const handleSaveQuestion = async () => {
    ...
    try {
      ...
      if (editingQuestion) {
        ...
        setQuestions((prev) =>
          prev.map((q) => (q.id === editingQuestion.id ? { ...q, ...questionForm } : q))
        );
      } else {
    ...
    <div className="space-y-6">
                  ...
                  <li key={c.id}>
                    <button
                      ...
                      className={`flex w-full items-center justify-between px-4 py-3 text-left text-sm transition-colors hover:bg-muted/60 ${
                        selectedChecklistId === c.id ? "bg-muted font-medium" : ""
                      }`}
                    >
          ...
          <CardHeader className="pb-3">
            {selectedChecklistId ? (
              <>
                ...
                <CardDescription>
                  Clause, subclause, requirement, question, and evidence example for each item.
                </CardDescription>
              </>
            ) : (
              <CardTitle className="text-base text-muted-foreground">Select a checklist</CardTitle>
            )}
          </CardHeader>
          <CardContent>
            {!selectedChecklistId ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground">Loading questions…</div>
            ) : questions.length === 0 ? (
                  ...
                  <TableBody>
                    {questions.map((q) => (
                      <TableRow key={q.id}>
                        ...
                        <TableCell className="max-w-[200px] truncate text-sm" title={q.requirement}>
                          {q.requirement}
                        </TableCell>
      ...
      <Dialog open={isCreateChecklistOpen} onOpenChange={setIsCreateChecklistOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsCreateChecklistOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={isEditChecklistOpen} onOpenChange={setIsEditChecklistOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEditChecklistOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={isQuestionDialogOpen} onOpenChange={setIsQuestionDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Clause, subclause, requirement, question text, and evidence example.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
      ...
      <Dialog open={!!deleteChecklistId} onOpenChange={() => setDeleteChecklistId(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will remove the checklist and all its questions.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
